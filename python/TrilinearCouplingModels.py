...
class DifferenialKappaLambda(LHCHCGBaseModel):
    def __init__(self, BRU=False):
        LHCHCGBaseModel.__init__(self)
        self.doBRU = BRU
        self.ScalingFunctions = {}
        self.ScaledProcs = []
    ...
    def doParametersOfInterest(self):
        ...
        self.modelBuilder.doVar("kappa_lambda[1,-30,30]")
        pois = "kappa_V,kappa_F,kappa_lambda"
        self.doMH()
        self.modelBuilder.doSet("POI", pois)
        self.SMH = SMHiggsBuilder(self.modelBuilder)
    ...
    def setup(self):
        ...
        print("Making production equations")
        for production in SM_HIGG_PROD:
            print("Building scaling for ", production)
            ...
            if production == "ggH":
                ...
                self.ScalingFunctions[production] = "kVkFkl_XSscal_%s_%s" % (
                    production,
                    energy,
                )
            print(self.ScalingFunctions)

    def getYieldScale(self, bin, process):
        "Split in production and decay, and call getHiggsSignalYieldScale; return 1 for backgrounds"
        #print("Scaling {} in bin {}".format(process, bin))

        if not self.DC.isSignal[process]:
            return 1

        processSource = process.split("_")[0]
        if processSource == "smH":
            processSource = "ggH"
        foundDecay = bin.split("_")[0]
        foundEnergy = "13TeV"
        
        if len(self.ScaledProcs) > 0 and not any(processSource.startswith(ScaledProc) for ScaledProc in self.ScaledProcs):
            return 1
        else:
            return self.getHiggsSignalYieldScale(processSource, foundDecay, foundEnergy, process, bin)

    def getHiggsSignalYieldScale(self, processSource, decay, energy, process, bin):
        production = processSource

        if production in ["ggH"]:
            XSscal = self.ScalingFunctions[production]
            ...
            print("Will scale process {} in bin {} with scaling function {}".format(process, bin, XSBRscaling))

            # print "XSBRscal = ", XSBRscaling, type(XSBRscaling)
            return XSBRscaling
        else:
            print("Will scale process {} in bin {} with 1".format(process, bin))
            return 1


differentialKappaLambda = DifferenialKappaLambda()
