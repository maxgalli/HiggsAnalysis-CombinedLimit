...
int main(int argc, char **argv) {
  ...
  combiner.ioOptions().add_options()("name,n",
      ...
      "dataset,D",
      po::value<string>(&dataset)->default_value("data_obs"),
      "Name of the dataset for observed limit - use this to replace dataset in workspace for example with a toy "
      "dataset. Format as file:workspace:object or file:object")(
      "dataMapName",
      po::value<string>(&dataMapName)->default_value("data_obs"),
      "Name of the dataset for observed limit pattern in the datacard")(
      "toysFile",
  ...
  if (seed == -1) {
    if (verbose > 0)
      std::cout << ">>> Using OpenSSL to get a truly random seed " << std::endl;
